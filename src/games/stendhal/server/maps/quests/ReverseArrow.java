...
public class ReverseArrow extends AbstractQuest implements
	 ...
	 * Checks the result.
	 */
	public class ReverseArrowCheck implements TurnListener {
		...
		public boolean checkBoard() {
			...
			for (int i = 1; i <= 3; i++) {
				...
				if (token.getX() != topX - 1 + i - 1
					...
					if (tokens.get(1).getX() != topX -1 || tokens.get(1).getY() != topY + 1 || 
							tokens.get(2).getX() != topX +1 || tokens.get(2).getY() != topY + 1 || 
							tokens.get(3).getX() != topX -2 || tokens.get(3).getY() != topY +2){
							return false;
						}
				}
			}
			...
			for (int i = 4; i <= 8; i++) {
				...
				if (token.getX() != topX - 2 + i - 4
						|| token.getY() != topY + 2) {
					if (tokens.get(4).getX() != topX -1 || tokens.get(4).getY() != topY +2 || 
						tokens.get(5).getX() != topX +0 || tokens.get(5).getY() != topY +2 || 
						tokens.get(6).getX() != topX +1 || tokens.get(6).getY() != topY +2 ||
						tokens.get(7).getX() != topX +2 || tokens.get(7).getY() != topY +2 ||
						tokens.get(8).getX() != topX +0 || tokens.get(8).getY() != topY +3){
						return false;
					}
				}
			}
	 ...
	 *            y-position
	 */
	public void addTokenToWorld(final int x, final int y) {
	...
	public boolean getCheckBoard() {
		ReverseArrowCheck checker = new ReverseArrowCheck();
		return checker.checkBoard(); //errored
	}
}
